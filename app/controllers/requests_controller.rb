class RequestsController < ApplicationController
  ...
  def create
    ...
    if @item.present?
      ...
      Message.create(content: "Nouvelle demande d'objet : #{strip_tags(@request.needed_item.capitalize)}. #{link_to 'Voir détails de la demande', dashboard_path(), class: "btn mt-3 ms-1"}", chatroom_id: "1", user: User.first)
    end
